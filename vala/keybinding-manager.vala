/*
valac --pkg gtk+-2.0 --pkg x11 --pkg gdk-x11-2.0 --pkg gee-1.0
keybinding-manager.vala
*/
...
class KeybindingManager : GLib.Object {
 ...
 private
  class Keybinding {
   public
    Keybinding(string accelerator, int keycode, Gdk.ModifierType modifiers,
  ...
  KeybindingHandlerFunc(Gdk.Event event);
 ...
 public
   ...
   */
 public
  void bind(string accelerator, KeybindingHandlerFunc handler) {
    ...
    if (keycode != 0) {
      ...
      // store binding
      Keybinding binding =
          new Keybinding(accelerator, keycode, modifiers, handler);
   ...
   */
 public
  void unbind(string accelerator) {
   ...
   */
 public
  Gdk.FilterReturn event_filter(Gdk.XEvent gdk_xevent, Gdk.Event gdk_event) {
...
private
static void test() { debug("hotkey pressed"); }
