...
class ClutterDemo {
 ...
 private
  ...
  const string[] colors = {
      "blanched almond", "OldLace",           "MistyRose",     "White",
      "LavenderBlush",   "CornflowerBlue",    "chartreuse",    "chocolate",
      "light coral",     "medium violet red", "LemonChiffon2", "RosyBrown3"};
 ...
 public
  void start() {
    ...
    for (int i = 0; i < rectangles.length; i++) {
      animations[i] =
          rectangles[i].animate(AnimationMode.LINEAR, 2000, x
                                : stage.width / 2, rotation_angle_z : 500.0);
    }
    animations[animations.length - 1].completed.connect(() = > {
      var text = new Text.full("Bitstream Vera Sans 40", "Congratulations!",
      ...
      var animation = text.animate(AnimationMode.EASE_OUT_BOUNCE, 1000, y
                                   : stage.height / 2);
      // animation.completed.connect(Gtk.main_quit);

      for (int i = 0; i < rectangles.length; i++) {
        rectangles[i].animate(
            AnimationMode.EASE_OUT_BOUNCE, 3000, x
            : Random.next_double() * stage.width, y
            : Random.next_double() * stage.height / 2 + stage.height / 2,
              rotation_angle_z
            : rectangles[i].rotation_angle_z, opacity : 0);
      }
    });
  }
}
