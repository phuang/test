...
class Application {
    ...
    m_drawing_area.motion_notify_event.connect((e) => {
      ...
      if (m_paths.length > 0) {
        m_paths[m_paths.length - 1].Add (Gdk.Point() {
          x = (int)(e.x_root * 400 / 1440),
          y = (int)(e.y_root * 400 / 900)
        });
  ...
  private void grab() {
    ...
    var devices = device_manager.list_devices(Gdk.DeviceType.MASTER);
    foreach (var dev in devices) {
      dev.grab(m_drawing_area.get_window(),
               Gdk.GrabOwnership.WINDOW,
               false,
               Gdk.EventMask.POINTER_MOTION_MASK |
               Gdk.EventMask.BUTTON_PRESS_MASK |
               Gdk.EventMask.KEY_PRESS_MASK,
               new Gdk.Cursor.for_display(Gdk.Display.get_default(), Gdk.CursorType.BLANK_CURSOR),
               Gdk.CURRENT_TIME);
    }
    ...
    try {
      GLib.Process.spawn_command_line_sync("/usr/bin/xinput set-mode 11 ABSOLUTE");
    } catch (GLib.Error e) {
      debug("xinput error");
    }
  }
  ...
  private void ungrab() {
    ...
    try {
      GLib.Process.spawn_command_line_sync("/usr/bin/xinput set-mode 11 RELATIVE");
    }catch(GLib.Error e) {
      debug("xinput error");
    }
  ...
  private bool drawing_area_draw(Cairo.Context cr) {
